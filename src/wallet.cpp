...
// PrimeChain: optional setting to unlock wallet for block minting only;
...
// PrimeChain: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // PrimeChain: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // PrimeChain: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // PrimeChain: not avatar mode
                        {
...
// PrimeChain: create coin stake transaction
...
// PrimeChain: check 'spent' consistency between wallet and txindex
// PrimeChain: fix wallet spent state according to txindex
...
// PrimeChain: disable transaction (only for coinstake)
