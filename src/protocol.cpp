...
// PrimeChain message start (switch from Bitcoin's in v0.2)
static unsigned char pchMessageStartBitcoin[4] = { 0x1a, 0x1b, 0x1c, 0x1d };
static unsigned char pchMessageStartPrimeChain[4] = { 0x2a, 0x2b, 0x2c, 0x2d };
...
void GetMessageStart(unsigned char pchMessageStart[], bool fPersistent)
{
    if (fTestNet)
        ...
        memcpy(pchMessageStart, (fPersistent || GetAdjustedTime() > nMessageStartSwitchTime)? pchMessageStartPrimeChain : pchMessageStartBitcoin, sizeof(pchMessageStartPrimeChain));
}
